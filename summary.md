...
1. **Традиционный подхдод (if-elif-else)**:
   ...
   - Если ни одно из условий не выполлнено. Используется else как дефолтный вариант
     (например, определление роли как "уборщица").
2. **Новая конструкция match-case**:
   ...
   - Match используется для переменной, которую надо сравинь. Case определляет варианты (например, 'admin', 'manager').
...
4. **Рекомендации**:
   ...
   - Для более слложных условий, вкючая дополнительные проверки, классический if-elif-else может оказаться более
     подходящим.
   - обязательно включение дефолтного кейса для обработки всех возможных значений.

### Линтер и форматер

В этой лекии обсуждаются инструменты длля автоматического соблюдения стандартов форматирования кода в Python при
