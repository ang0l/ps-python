...
В этой лекции я завершаю создание приложения, добавляя последнюю команду - `done`. Она будет обозначать задачу как
выполненную и, если она уже выполнена, изменять статус обратно на "новую".

Основные шаги:

1. **Создание команды `done`**:
   - создаю новый модуль `done.py`, аналогично предыдущим командам;
   - разрабатываю функцию `done_command()`, которая принимает аргументы задач.
2. **Обработка аргументов**:
   - проверяется наличие аргументов; если их нет, выводится инструкция по использованию и завершается выполнение.
3. **Проверка и идентификация задачи**:
   - исспользуется `try-exept` для обработки возможных ошибок при конверсии ID задачи;
   - ищется задача в списке с помощью функции `find_tasks()`.
4. **Изменение статуса задачи**:
   - если задача "новая" (`new`), меняется статус на "выполненная" (`done`);
   - если задача уже выполнена, меняется статус обратно на "новая".
5. **Вывод результата**:
   - используется функция `stringify_table()` для отображения обновленной задачи.

### Следующие улучшения

В текущем проекте я создал функциональную утилиту для управления задачами, но есть возможности для ее дальнейшего
улучшения:

1. **Фильтрация задач**: добавить возможность фильтровать задачи по статусу - например, "выполненные" и
   "невыполненные".
2. **Группировка задач**: реализовать группировку задач по тегам, например, по приоритету ("важно") или по проектам;
   можно создать отдельные поля для проектов и отображать задачи по проектам.
3. **Поля задачи**: ввести новое поле `created_at`, фиксирующее дату создания задачи, чтобы отслеживать динамику
   выполнения.
4. **Дополнительные идеи**: развиватиь утилиту на основе собственных потребностей и предожений.

Эти улучшения помогут сделать утилиту более гибкой и полезной. Я могу сам выбрать, какие из пунктов реализовать.

## Раздел 17. Заключение

### Куда двигаться дальше

Принимаю поздравления с успешным завершение курса по основам Python! Этот курс был первым шагом на пути к работе с веб-
приложениями и машинным обучением.

#### Что дальше

Следующий курс будет более продвинутым

Темы: создание полноценного API на Python и объектно-ориентиированное программирование.

#### Как продолжить обучение

Найти курс в каталоге, выбрав направление Python.

Рассмотреть другие направления, такие как Golang и Node.js, для расширения навыков.

Надеются встретиться со мной в следующем курсе!
